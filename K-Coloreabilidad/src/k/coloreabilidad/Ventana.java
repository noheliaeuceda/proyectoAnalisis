...
public class Ventana extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jButton3.setText("K-Coloreabilidad");
...
//                JOptionPane.showMessageDialog(this, "Se a creado la arista Exitosamente");
    ...
    ArrayList<Node> grafoGlobal = new ArrayList();
    ...
    public void dibujarGrafo(ArrayList<Node> grafotemp) {
        ...
        for (int i = 0; i < grafotemp.size(); i++) {
            ...
            if (grafotemp.get(i).getIdColor() == 0) {
                ...
                complementaria.add(Graph.insertVertex(Parent, null, grafotemp.get(i), 70 * i, y, 50, 50, "fillColor=blue"));
            } else if (grafotemp.get(i).getIdColor() == 2) {
                complementaria.add(Graph.insertVertex(Parent, null, grafotemp.get(i), 70 * i, y, 50, 50, "fillColor=white"));
            } else if (grafotemp.get(i).getIdColor() == 3) {
                complementaria.add(Graph.insertVertex(Parent, null, grafotemp.get(i), 70 * i, y, 50, 50, "fillColor=red"));
            } else if (grafotemp.get(i).getIdColor() == 4) {
                complementaria.add(Graph.insertVertex(Parent, null, grafotemp.get(i), 70 * i, y, 50, 50, "fillColor=orange"));
            } else if (grafotemp.get(i).getIdColor() == 5) {
                complementaria.add(Graph.insertVertex(Parent, null, grafotemp.get(i), 70 * i, y, 50, 50, "fillColor=yellow"));
            } else if (grafotemp.get(i).getIdColor() == 6) {
                complementaria.add(Graph.insertVertex(Parent, null, grafotemp.get(i), 70 * i, y, 50, 50, "fillColor=green"));
            }
        }
    ...
    public void MCIAlgoritm(ArrayList<Node> graph) {
        MCI(graph);
        count++;
        ArrayList<Node> deleteGraph = new ArrayList();
        deleteGraph = cargar();
        for (int i = 0; i < set.size(); i++) {
            for (int j = 0; j < grafoGlobal.size(); j++) {
                if (grafoGlobal.get(j).getValue() == set.get(i).getValue()) {
                    grafoGlobal.get(j).setIdColor(count);
                }
            }
        }
        for (int i = 0; i < set.size(); i++) {
            for (int j = 0; j < deleteGraph.size(); j++) {
                if (deleteGraph.get(j).getValue() == set.get(i).getValue()) {
                    removeNode(deleteGraph, j);
                }
            }
        }
        set = new ArrayList();
        if (!deleteGraph.isEmpty()) {
            guardar(deleteGraph);
            MCIAlgoritm(deleteGraph);
        } else {
        }
    }

    public void MCI(ArrayList<Node> graph) {
        ArrayList<Node> copyGraph = graph;
        for (int i = 0; i < copyGraph.size(); i++) {
            copyGraph.get(i).setGrado();
            copyGraph.get(i).setID(i);
        }
        int minGrade = copyGraph.size();
        int index = 0;
        int value;
        for (int i = 0; i < copyGraph.size(); i++) {
            if (minGrade > copyGraph.get(i).getGrado()) {
                minGrade = copyGraph.get(i).getGrado();
                index = i;
            }
        }
        value = copyGraph.get(index).getValue();
        set.add(copyGraph.get(index));
        int size = copyGraph.get(index).getAristas().size();
        for (int h = 0; h < size; h++) {
            int subIndex = copyGraph.get(index).getAristas().get(h).getNodoFinal().getID();
            removeNode(copyGraph, subIndex);
            for (int i = 0; i < copyGraph.size(); i++) {
                if (value == copyGraph.get(i).getValue()) {
                    index = i;
                }
            }
            size = copyGraph.get(index).getAristas().size();
            h = -1;
        }
        removeNode(copyGraph, index);
    ...
    public void removeNode(ArrayList<Node> copyGraph, int index) {
        for (int i = 0; i < copyGraph.size(); i++) {
            for (int j = 0; j < copyGraph.get(i).getAristas().size(); j++) {
                if (copyGraph.get(i).getAristas().get(j).getNodoFinal().getID() == copyGraph.get(index).getID()) {
                    removeEdge(copyGraph, copyGraph.get(i), copyGraph.get(i).getAristas().get(j));
                } else if (copyGraph.get(i).getAristas().get(j).getNodoInicial().getID() == copyGraph.get(index).getID()) {
                    removeEdge(copyGraph, copyGraph.get(i), copyGraph.get(i).getAristas().get(j));
                }
            }
        }
        copyGraph.remove(index);
        for (int i = 0; i < copyGraph.size(); i++) {
            copyGraph.get(i).setID(i);
        }
    }

    public void removeEdge(ArrayList<Node> tempGrafo, Node vertex, Arista edge) {
        for (int i = 0; i < tempGrafo.size(); i++) {
            if (vertex.getID() == (tempGrafo.get(i)).getID()) {
                for (int j = 0; j < (tempGrafo.get(i)).getAristas().size(); j++) {
                    if (((tempGrafo.get(i)).getAristas().get(j)).getNodoInicial().getID()
                            == edge.getNodoInicial().getID()
                            && ((tempGrafo.get(i)).getAristas().get(j)).getNodoFinal().getID()
                            == edge.getNodoFinal().getID()) {
                        (tempGrafo.get(i)).getAristas().remove(j);
                        break;
                    }
                }
            }
